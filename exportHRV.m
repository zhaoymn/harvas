...
%% Get total number of hrv variables and variable names
    ...
    % MSE vars
    varM=fieldnames(hrv(f).mse);
    lenM=length(varM); % times 3 because of 3 freq domain methods
...
%% Build row header names
    for c=1:lenV
        ...
        elseif c<=(lenIBI+lenT+lenM) && c>(lenIBI+lenT)                    %MSE
            c2=c-lenIBI-lenT;
            %insert method name
            if (c2-1)==0
                output{1,c+1}='MSE';
...
%                     output{3,c+1}='(ms)';
%             end
        elseif c<=(lenIBI+lenT+lenM+lenF) && c>(lenIBI+lenT+lenM)        %Freq Domain
            c2=c-lenIBI-lenT-lenM;
            %insert method name
        ...
        elseif c<=(lenIBI+lenT+lenM+lenF+lenP)...
                    && c>(lenIBI+lenT+lenM+lenF)                       %Poincare
            c2=c-lenIBI-lenT-lenM-lenF;
            %insert method name
            if (c2-1)==0
            ...
            %insert units            
            output{3,c+1}='(ms)';
        elseif c<=(lenIBI+lenT+lenM+lenF+lenP+lenNL)...
                    && c>(lenIBI+lenT+lenM+lenF+lenP)                 %Nonlinear
            c2=c-lenIBI-lenT-lenM-lenF-lenP;
            %insert method name
            if (c2-1)==0
        ...
        elseif c<=(lenIBI+lenT+lenM+lenF+lenP+lenNL+lenTF) ...
                    && c>(lenIBI+lenT+lenM+lenF+lenP+lenNL)      %Time-Freq
            c2=c-lenIBI-lenT-lenM-lenF-lenP-lenNL;
            %insert method name
...
%% Add HRV to output array
    for f=1:lenS %rows/subjects
        for c=1:lenV %cols
            ...
            elseif c<=(lenIBI+lenT+lenM) && c>(lenIBI+lenT)                %MSE
                c2=c-lenIBI-lenT;
                varName=varM{c2};
                output{f+3,c+1}=hrv(f).mse.(varName);                
            elseif c<=(lenIBI+lenT+lenM+lenF) ...
                    && c>(lenIBI+lenT+lenM)                         %Freq Domain
                c2=c-lenIBI-lenT-lenM;
    ...
    for c=1:length(hrv.mse.MSE)
        output{lenS+5,c+1}=hrv(f).mse.MSE(c);
    end
