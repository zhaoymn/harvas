...
pks = y(iPk);
locs = x(iPk);
deletearray = ones(size(pks));
lastpos = -100;
for i=1:1:length(deletearray)
    if locs(i) - lastpos < Pd
        deletearray(i) = 0;
    else
        lastpos = locs(i);
    end
end
idxlength = length(find(deletearray==1));
idx = zeros(idxlength,1);
idxind = 0;
idxs = 1:1:length(deletearray);
for i=1:1:length(deletearray)
    if deletearray(i) == 1
        idxind = idxind +1;
        idx(idxind) = idxs(i);
