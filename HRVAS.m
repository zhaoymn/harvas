...
%         'Callback', @btnRead_Callback);
    ...
    h.btnmove_left = uicontrol(h.panelBTN,'Style','pushbutton', 'String','<<<',...
        'Units', 'normalized', 'Position',[.02 .80 1 0.1],...
        'Callback', @btnmoveleft_Callback);
    h.btnmove_right = uicontrol(h.panelBTN,'Style','pushbutton', 'String','>>>',...
        'Units', 'normalized', 'Position',[.02 .65 1 0.1],...
        'Callback', @btnmoveright_Callback);
    ...
    function btnmoveleft_Callback(hObject, eventdata)
        if showpos < showlength/6*sample_rate+1
            showpos = 1;
        else
            showpos = showpos - showlength/6*sample_rate;
        end
        slidervalue = showpos;
        axes(h.axesECG);
        cla reset
        plotQRS(h,ECG);
        h.slidercontrol.set('value',slidervalue);
        axes(h.axesIBI);
        cla reset
    ...
    function btnmoveright_Callback(hObject, eventdata)
        if showpos > length(ECG(:,1)) - showlength/6*sample_rate
            showpos = length(ECG(:,1)) - showlength*sample_rate + 1;
        else
            showpos = showpos + showlength/6*sample_rate;
        end
        slidervalue = showpos;
...
%         end
        ...
        if length(y0)>sample_rate*showlength && firstplot == 0
            set(h.slidercontrol,'value',1,'max',length(ECG(:,1))-sample_rate*showlength,'min',1,'Enable','on',...
                'sliderstep',[showlength*sample_rate/length(ECG(:,1))/6,showlength/length(ECG(:,1))],...
                'callback',@slidermove);
            stop(h.slidertimer);
            start(h.slidertimer);
            firstplot = 1;
