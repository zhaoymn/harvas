function [qrs_amp_raw,qrs_time_raw,qrs_i_raw,delay,RR]=pan_tompkin2(ecg,fs,gr)
...
% %% 预先剔除原始ecg信号中电压值不在（-1000，2000）的异常点
% position1=find(ecg(:)>2000);
% ecg(position1)=[];
% position2=find(ecg(:)<-1000);
% ecg(position2)=[];
...
post_qrs_i_raw=[];
post_qrs_i_raw(1)=qrs_i_raw(1);
m=1;
for k=1:length(qrs_i_raw)-1
       if qrs_i_raw(k+1)-post_qrs_i_raw(m)>0.2*fs
          post_qrs_i_raw(m+1)=qrs_i_raw(k+1);
          m=m+1;
       end
end
post_qrs_i_raw=post_qrs_i_raw';

% figure;
% plot(ecg);
% hold on;
% plot(qrs_i_raw,ecg(qrs_i_raw),'g+');
% hold on;
% plot(post_qrs_i_raw,ecg(post_qrs_i_raw),'ro');

qrs_i_raw=post_qrs_i_raw;
% 获得QRS波出现的时刻
...
% plot(ecg);
% hold on;
% plot(qrs_i_raw,ecg(qrs_i_raw),'ro');
end
