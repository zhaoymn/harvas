...
%即ibi为最原始的剔除异常RR间期前且去趋势前的；nibi为剔除异常RR间期后去趋势前的；dibi为去趋势后的
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    p.addParamValue('polyOrder', '1st Order', ...
        @(x)any(strcmpi(x,{'1st Order','2nd Order','3rd Order'})));
...
function [nibi,art]=correctEctopic(ibi,opt)
    y=ibi(:,2);   %ibi为最原始的剔除异常RR间期前且去趋势前的
    t=ibi(:,1);
    %locate ectopic
...
function [dibi,nibi,trend]=detrendIBI(ibi,opt)
    ...
    switch lower(opt.detrendMethod)
        ...
        case 'polynomial' %linear detrend
            ...
            t2=(t-mean(t))./std(t);
            %fit
            if opt.polyOrder == '1st Order'
                n=1;
            elseif opt.polyOrder == '2nd Order'
                n=2;
            elseif opt.polyOrder == '3rd Order'
                n=3;
            end
            [p,S]= polyfit(t2,y,n);   %t2是自变量，y是待拟合的因变量，n是多项式的阶数；p(x)=p1*x^n+p2*x^(n-1)+...+pn*x+p(n+1)
